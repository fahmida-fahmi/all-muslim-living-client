...
const FavLists = () => {
  const [favLists, refetch] = useFavLists()
  const [userData] = useUsers()
  const [biodatas] = useBiodatasInfo()
  const navigate = useNavigate()

  const userFavLists = favLists
  ...
  const deleteBtn = (user) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        fetch(`https://all-muslim-living-server.onrender.com/favLists/${user._id}`, {
          method: 'DELETE'
        })
          .then(res => res.json())
          .then(data => {
    ...
    <div className="w-full px-5">
      <div className="">
        <div className="flex items-center mb-6">
          ...
          <h1 className="text-3xl font-semibold text-purple-800 text-center flex-grow">Favorite List</h1>
        </div>
        ...
        <div className="bg-white shadow-md rounded-lg overflow-hidden">
          <table className="min-w-full text-sm text-left text-gray-700">
            ...
            <tbody>
              {userFavLists.length === 0 ? (
                <tr key={userFavLists.index} className="border-t text-center">
                  <td colSpan="5" className="px-6 py-4 text-center text-gray-500">
                    No data available
                  </td>
                </tr>
              ) : (
                userFavLists.map((user, index) => (
